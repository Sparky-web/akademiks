...
import updateSchedule, { ResultItem, UpdateReport } from "./_lib/utils/update-schedule";
...
export default createTRPCRouter({
    ...
    get: publicProcedure.input(z.object(
        ...
        if (input.groupId) {
            return await getStudentSchedule(input.groupId, input.weekStart, isAdmin)
        }
    ...
    update: protectedProcedure.input(z.object({
        ...
        const report = await updateSchedule(input.updates, input.shouldDisplayForStudents)
    ...
    updateFromAdmin: protectedProcedure.input(z.object({
        shouldDisplayForStudents: z.boolean(),
        updates: z.any()
    })).mutation(async ({ ctx, input }) => {
        if (!ctx.session?.user?.isAdmin) throw new TRPCClientError('Доступ запрещен')
            ...
            try {
                const data = await ctx.db.lesson.create({
                    data: {
                        ...
                        startDay: DateTime.fromJSDate(lesson.start).startOf('day').toJSDate(),
                        index: lesson.index,
                        subgroup: lesson.subgroup ? lesson.subgroup : null,
                        teacherId: lesson.teacherId,
                        groupId: lesson.Group?.id,
        ...
        try {
            notificationResult = await notifyFromReports(reports, input.shouldDisplayForStudents)
        } catch (e) {
    ...
    changeVisibility: protectedProcedure.input(z.object({
        dstart: z.date(),
        dend: z.date(),
        shouldDisplayForStudents: z.boolean()
    })).mutation(async ({ ctx, input }) => {
        ...
        if (shouldDisplayForStudents) {
            try {
                updateReport.notificationResult = await notifyFromReports(report, shouldDisplayForStudents, false)
            } catch (e) {
                console.error('Ошибка отправки уведомлений: ' + e.message)
                return 'ok'
            }
        ...
        updateReport = generateReport(updateReport, report, updateReport.notificationResult)
    ...
    getIsVisible: protectedProcedure.input(z.object({
        start: z.date(),
        end: z.date(),
