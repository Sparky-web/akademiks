...
export type SanitizedOptions = Required<
  NonNullable<DataTableProps<any>["options"]>
>;
...
function _DataTable({ useTableContext }: TablePassedProps) {
  ...
  const _rows = table
    .getRowModel()
    .rows.filter((row) => (showOnlySelected ? row.getIsSelected() : true));
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: (index) => {
      if (!options.virtualization.dynamicRowHeight?.enabled)
        return options.virtualization.fixedRowHeight || 85;
  ...
  return (
    <div className={cn("grid gap-4", options.size === "sm" && "text-xs")}>
      {(options.header?.rowCount ||
        options.header?.search ||
        options.header?.columnManagement) && (
        <DataTableToolbar
      ...
      <div
        className={cn(
          "relative rounded-xl",
          "bg-card !h-[75dvh] overflow-auto",
        )}
        ref={tableContainerRef}
      >
        <Table className={options.virtualization.enabled ? "table-fixed" : ""}>
          <TableHeader
            className="bg-muted sticky top-[-1px] z-10 shadow-md"
            style={
              ...
              >
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead
                      className={cn(
                        options.size === "sm" && "px-3 py-1 text-xs",
                        "relative w-full",
                      ...
                      style={{
                        ...
                        display: options.virtualization.enabled
                          ? "inline-flex"
                          : "",
                ...
                {rowVirtualizer.getVirtualItems().map((virtualRow, index) => {
                  ...
                  return (
                    <TableRow
                      ...
                      //   className={
                      //     options.pinning.enabled &&
                      //     "inline-flex h-fit items-center"
                      //   }
                      ...
                      {row.getVisibleCells().map((cell) => (
                        <TableCell
                          ...
                          className={cn(
                            ...
                            // (cell.column.columnDef as ExtendedColumnDef<any>)
                            //   .options?.editProps?.enabled &&
                            //   "p-0 outline outline-1 outline-transparent focus-within:outline-slate-300",
                            "p-3 text-sm",
                            // options.size === "sm" && "px-3 py-2 text-xs",
                        ...
                        >
                          ...
                          (cell.column.columnDef as ExtendedColumnDef<any>)
                            .options?.editProps?.enabled &&
                          (options.editable?.getRowCanEdit
                            ? options.editable?.getRowCanEdit(row)
                            : true)
                            ? flexRender(
                                (
                                  cell.column
                                    .columnDef as ExtendedColumnDef<any>
                                ).options?.editProps?.customCell ||
                                  EditableCell,
                                { ...cell.getContext(), useTableContext },
                              )
                    ...
                    {row.getVisibleCells().map((cell) => (
                      ...
                      >
                        ...
                        (options.editable?.getRowCanEdit
                          ...
                          ? flexRender(
                              (cell.column.columnDef as ExtendedColumnDef<any>)
                                .options?.editProps?.customCell || EditableCell,
                              { ...cell.getContext(), useTableContext },
                            )
                          : flexRender(cell.column.columnDef.cell, {
                              ...cell.getContext(),
...
function __DataTable(props: DataTableProps<any>) {
  ...
  const columnsTransposed: ExtendedColumnDef<any>[] = useMemo(() => {
    const cols: ExtendedColumnDef<any>[] = props.columns.map((e) => ({
      filterFn: (
        ...
        if (
          columnDef.options?.filters?.array?.enabled &&
          Array.isArray(filterValue)
        ) {
          const values = row
            .getValue(columnId)
            .map((e) =>
              columnDef.options?.filters?.array.getFilterValue(e).toString(),
            );
          if (!values.length) return filterValue.includes("Не указан");
          return values.some((e) => filterValue.includes(e));
        }
        const value = column.columnDef.options?.filters?.getFilterValue
          ? column.columnDef.options?.filters?.getFilterValue(row)
    ...
    if (props.options?.actions?.enabled) {
      cols.push({
        ...
        cell: ({ row }) => {
          return (
            <DataTableActions rows={[row]} useTableContext={useTableContext} />
          );
        },
        enableHiding: false,
      });
  ...
  const table = useReactTable({
    ...
    rowCount: options.footer.serverPagination?.enabled
      ? options.footer.serverPagination.rowCount
      : data.length,
    state: {
      sorting,
      columnVisibility:
        (options.optionsSaver?.enabled ? settings?.visibility : null) ||
        columnVisibility,
      rowSelection,
      columnFilters,
      columnSizing:
        (options.optionsSaver?.enabled ? settings?.sizing : null) ||
        columnSizing,
      // pagination: options.footer.serverPagination?.enabled ? options.footer.serverPagination.state : pagination,
      pagination: options.footer.pagination?.enabled
        ? pagination
        : options.footer.serverPagination?.enabled
          ? options.footer.serverPagination.state
    ...
    onSortingChange: options.serverSorting?.enabled
      ? options.serverSorting.onSortingChange
      : setSorting,
    ...(options.serverSorting?.enabled && { manualSorting: true }),
    onColumnFiltersChange: setColumnFilters,
    onColumnVisibilityChange: setColumnVisibility,
    onPaginationChange: options.footer.serverPagination?.enabled
      ? options.footer.serverPagination.onPaginationChange
      : setPagination,
    onColumnSizingChange: setColumnSizing,
    manualPagination: options.footer.serverPagination?.enabled,

    enableRowSelection: options.selectable.enabled
      ? (row) =>
          typeof row.original.isSelectable === "undefined"
            ? true
  ...
  useEffect(() => {
    if (hooks?.onRowSelectionChange)
      hooks.onRowSelectionChange(table.getSelectedRowModel().rows);
  ...
  const useTableContext = useMemo(
    () => ({ ...props, options: optionsMerged, table }),
    [],
  );

  return <_DataTable useTableContext={useTableContext} />;
}
export default __DataTable;

const getCommonPinningStyles = (column: Column<any>): CSSProperties => {
  const isPinned = (column.columnDef as ExtendedColumnDef<any>).options?.pinning
    ?.enabled;
