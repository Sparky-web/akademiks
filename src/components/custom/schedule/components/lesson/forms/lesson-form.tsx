...
export default function CreateLessonForm({
  ...
  const { data: lessonTitles } = api.handbooks.getUniqueLessonTitles.useQuery(
    type === "teacher"
      ? { teacherId: field.state.value.Teacher?.id }
      : { groupId: field.state.value.Group?.id },
  );
  ...
  const teachersFlattened = teachers?.flatMap((e) =>
    "values" in e ? e.values : [e],
  );
  const classroomsFlattened = classrooms?.flatMap((e) =>
    "values" in e ? e.values : [e],
  );
  const groupsFlattened = groups?.flatMap((e) =>
    "values" in e ? e.values : [e],
  );

  return (
    <form
      className="grid w-[200px] content-start gap-1 pb-10"
      onSubmit={(e) => {
        e.preventDefault();
        // form.handleSubmit()
      }}
    >
      ...
      {type === "student" && (
        <form.Field name={`${lessonFormKey}.teacherId`}>
          {(field) => {
            return (
              <LabelGroup>
                ...
                <Combobox
                  ...
                  onChange={(value) => {
                    ...
                    form.setFieldValue(`${lessonFormKey}.Teacher`, {
                      id: value,
                      name: teachersFlattened?.find((e) => e.value === value)
                        ?.label,
                    });
                  }}
                ></Combobox>
            ...
            <Combobox
              ...
              onChange={(value) => {
                ...
                form.setFieldValue(`${lessonFormKey}.Classroom`, {
                  id: value,
                  name: classroomsFlattened?.find((e) => e.value === value)
                    ?.label,
                });
              }}
            ></Combobox>
